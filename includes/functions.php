...
//    var_dump($project_user_role, $project_id);
    // grant project manager all access
    // also if the user role has the ability to manage all projects from settings, allow him
...
function cpm_project_count() {
    ...
    if ( $count === false ) {
        $sql = "SELECT COUNT(post_id) AS count, meta.meta_value AS type FROM {$wpdb->posts} AS post
            LEFT JOIN {$wpdb->postmeta} AS meta on meta.post_id = post.ID
            $project_category_join
            $role_join          
            WHERE 
                post.post_type='project' AND post.post_status = 'publish'
                $project_category
                $role_where
    ...
    <div class="cpm-settings-bind cpm-settings-icon-cog"><span><?php echo $action; ?></span></div>
        ...
        <ul class="cpm-right cpm-settings" >
            <li>
                ...
                <a href="<?php echo add_query_arg( array('page'=>'cpm_projects') ,get_permalink() ); ?>" class="cpm-project-delete-link" title="<?php esc_attr_e( 'Delete project', 'cpm' ); ?>" <?php cpm_data_attr( array('confirm' => __( 'Are you sure to delete this project?', 'cpm' ), 'project_id' => $project_id) ) ?>>
